...
namespace OMSServiceMini.Controllers
{
    ...
    public class AuthenticationController : Controller/////////////////////////////////////////////////////////////////////////////////////////////////////
    {
        ...
        public async Task<IActionResult> Register_Admin([FromBody] RegisterModel registerModel)
        {
            var userExists = await userManager.FindByNameAsync(registerModel.Username);
            if (userExists != null)
                return StatusCode(StatusCodes.Status500InternalServerError, new ReportModel { Message = "Такой пользователь существует" });
            ...
            if (!result.Succeeded)// ошибка была из-за отсутствия цифр в пароле, а так же в отсутствии заглавных букв в пароле
                return StatusCode(StatusCodes.Status500InternalServerError, new ReportModel { Message = "Ошибка 500, пользователя не получилось создать" });
            ...
            return Ok(new ReportModel { Message = "Создан пользователь с ролью Admin" });
        }
        ...
        public async Task<IActionResult> Register_User([FromBody] RegisterModel registerModel)
        {
            ...
            return Ok(new ReportModel { Message = "Создан пользователь с ролью User" });
        }
